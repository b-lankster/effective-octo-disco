#!/bin/bash
# build_gear_skills.sh — converts armor_skills.tsv into the GEAR_SKILLS + SKILL_NAMES
# blocks and patches them into js/data.js
# Usage: bash scripts/build_gear_skills.sh

set -e
cd ~/mhnow_companion

TSV="scripts/armor_skills.tsv"

if [ ! -f "$TSV" ]; then
  echo "ERROR: $TSV not found. Run scrape_armor.sh first."
  exit 1
fi

echo "Rows in TSV: $(wc -l < "$TSV")"

# ── 1. Fetch skill display names ──────────────────────────────────────────────
echo "Fetching skill display names..."

# TSV columns: equip_id(1) grade(2) skill_id(3) level(4)
mapfile -t SKILL_IDS < <(awk -F'\t' '{print $3}' "$TSV" | sort -u)
...
echo "  Skill names written: $(grep -c "':'" scripts/skill_names_generated.js) entries"

# ── 2. Build GEAR_SKILLS from TSV ─────────────────────────────────────────────
echo "Building GEAR_SKILLS..."
# Sort: equip_id(k1), skill_id(k3), grade numerically(k2)
sort -t$'\t' -k1,1 -k3,3 -k2,2n "$TSV" | \
...
# Find where the existing SKILL_NAMES block starts and truncate there
